...
AudioFormat::AudioFormat(const AVCodecContext *ctx)
{
	...
	if (ctx)
	{
		...
		if (ctx->ch_layout.order != AV_CHANNEL_ORDER_UNSPEC)
		{
			...
			av_channel_layout_default(&stereo, channelsNo);
			channelLayout = stereo.u.mask;
		}
		else
		{
			AVChannelLayout def;
			av_channel_layout_default(&def, channelsNo);
			channelLayout = def.u.mask;
		}
#else
...
#if LIBAVUTIL_VERSION_INT >= AV_VERSION_INT(57, 28, 100)
	AVChannelLayout layout = {};
	if (av_channel_layout_from_string(&layout, name) == 0)
