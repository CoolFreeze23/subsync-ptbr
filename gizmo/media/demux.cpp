...
static const AVInputFormat *getInputFormatByFname(const char *path);
	...
	m_runCb(runCb)
{
	...
	if (res < 0 && res != AVERROR_EXIT)
	{
		...
		const AVInputFormat *fmt = getInputFormatByFname(fileName.c_str());
...
bool Demux::step()
{
	...
	if (!packet)
		throw EXCEPTION("can't allocate AVPacket")
			.module("Demux", "av_packet_alloc");

	ScopeExit scopedPacketFreeGuard([&packet](){ av_packet_free(&packet); });
...
const AVInputFormat *getInputFormatByFname(const char *path)
{
